...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SubmitBlock(blockHex, callback) {
        ...
        if (options.hasSubmitMethod) {
            ...
            rpcArgs = [{'mode': 'submit', 'data': blockHex, "rules": ["segwit"]}];
        }
...
function GetBlockTemplate(callback) {
    ...
    _this.daemon.cmd(
        ...
        async function (results) {
            ...
            // Safety check for the entire RPC response
            if (!results || results.length === 0 || !results[0]) {
                var error = { code: -1, message: "RPC response is empty or invalid" };
                emitErrorLog('getblocktemplate call returned an invalid response structure.');
                return callback(error);
            }

            if (results[0].error) {
                emitErrorLog('getblocktemplate call failed with error: ' + JSON.stringify(results[0].error));
                return callback(results[0].error);
            }

            var rpcData = results[0].response;
            
            // Safety check for the data inside the response
            if (!rpcData) {
                var error = { code: -1, message: "RPC response data is null or undefined" };
                emitErrorLog('getblocktemplate call returned empty response data');
                return callback(error);
            }

            try {
                var processedNewBlock = await _this.jobManager.processTemplate(rpcData);
                callback(null, rpcData, processedNewBlock);
            } catch (e) {
                var error = { code: -1, message: "Error processing block template: " + e.message };
                emitErrorLog('Error processing block template: ' + (e.message || e));
                callback(error);
            }
        }
    );
