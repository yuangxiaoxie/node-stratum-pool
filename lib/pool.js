    ...
    _this.daemon.cmd(
        ...
        async function (results) {
            ...
            if (!results || results.length === 0 || !results[0] || results[0].error) {
                var error = (results && results[0]) ? results[0].error : { code: -1, message: "Unknown RPC error or empty response" };
                emitErrorLog('getblocktemplate call failed with error: ' + JSON.stringify(error));
                callback(error);
                return;
            }
