...
function DaemonInterface(daemons, logger) {
    ...
    var instances = (function () {
        var filteredDaemons = [];
        if (daemons && Array.isArray(daemons)) { // Check if daemons is a valid array
            filteredDaemons = daemons.filter(function (d) { 
                // Only keep daemons that are actual objects with host and port
                return d && typeof d === 'object' && d.host && d.port;
            });
            filteredDaemons.forEach(function (daemon, i) {
                daemon.index = i;
            });
        }
